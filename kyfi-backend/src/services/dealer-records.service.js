...
const listDealerFarmerRecords = async (dealerId) => {
  ...
  const [countActionRows] = await db.execute(
    `SELECT
      ...
      fsca.dealer_id,
      fsca.action_type,
      DATE_FORMAT(fsca.created_at, '%Y-%m-%d %H:%i:%s') AS created_at,
      DATE_FORMAT(fsca.updated_at, '%Y-%m-%d %H:%i:%s') AS updated_at,
      fsv.vote_color,
      DATE_FORMAT(fsv.updated_at, '%Y-%m-%d %H:%i:%s') AS voted_at,
      fs.aadhaar,
      fs.farmer_name,
      fs.mobile_number,
      fs.district,
