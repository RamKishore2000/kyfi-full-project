...
const listDealerFarmerRecords = async (dealerId) => {
  ...
  const [countActionRows] = await db.execute(
    `SELECT
      ...
      fsca.dealer_id,
      fsca.action_type,
      DATE_FORMAT(DATE_ADD('1970-01-01 00:00:00', INTERVAL (UNIX_TIMESTAMP(fsca.created_at) + 19800) SECOND), '%Y-%m-%d %H:%i:%s') AS created_at,
      DATE_FORMAT(DATE_ADD('1970-01-01 00:00:00', INTERVAL (UNIX_TIMESTAMP(fsca.updated_at) + 19800) SECOND), '%Y-%m-%d %H:%i:%s') AS updated_at,
      fsv.vote_color,
      DATE_FORMAT(DATE_ADD('1970-01-01 00:00:00', INTERVAL (UNIX_TIMESTAMP(fsv.updated_at) + 19800) SECOND), '%Y-%m-%d %H:%i:%s') AS voted_at,
      fs.aadhaar,
      fs.farmer_name,
      fs.mobile_number,
      fs.district,
