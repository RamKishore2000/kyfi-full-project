...
export async function fetchMandals(params: {
  ...
  const response = await fetch(`${KYFI_API_BASE_URL}/locations/mandals?${searchParams.toString()}`, {
    method: "GET",
    headers: {
  ...
  const data = (await response.json().catch(() => null)) as MandalsResponse | { message?: string } | null;
