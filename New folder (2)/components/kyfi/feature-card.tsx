...
export function FeatureCard({
  ...
  const content = (
    <Card className="group flex h-[7.75rem] overflow-hidden border-white/80 bg-white/85 shadow-[0_16px_50px_rgba(15,23,42,0.08)] transition-colors hover:border-emerald-200 sm:h-full">
      <CardContent className="flex h-full flex-1 flex-col justify-between gap-4 p-4 sm:gap-5 sm:p-6">
        ...
        <div className="space-y-2">
          ...
          {mobileDescription ? (
            <p className="font-manrope text-[0.72rem] font-semibold leading-4 text-slate-500 sm:hidden">
              {mobileDescription}
            </p>
          ) : null}
          {descriptionContent ? (
